...
from scheduler.forms import ØvedagForm
...
def tilføj_øvedag(request):
    # Get the context from the request.
    context = RequestContext(request)

    # A HTTP POST?
    if request.method == 'POST':
        form = ØvedagForm(request.POST)

        # Have we been provided with a valid form?
        if form.is_valid():
            # Save the new øvelsesdag to the database.
    ...
    return render_to_response('scheduler/tilfoj_ovedag.html', {'form': form}, context)
